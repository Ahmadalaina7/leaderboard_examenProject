...
export async function GET() {
  ...
  const rows = await db.all('SELECT * FROM scores ORDER BY score DESC, created_at ASC');
...
export async function POST(request: Request) {
  try {
    ...
    const db = await getDb();
    const result = await db.run('INSERT INTO scores (name, score) VALUES (?, ?)', [name, score]);
    const inserted = await db.get('SELECT * FROM scores WHERE id = ?', result.lastID);
